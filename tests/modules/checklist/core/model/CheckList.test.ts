...
describe('CheckList', () => {
  ...
  it('creates a checklist and emits a CheckListCreated event', () => {
    const checklist = CheckList.create(new IdCheckList(ID.generateId().getId()), owner, new Text('My checklist'), actor, key);
    const events = checklist.pullEvents();

    expect(checklist.getName().getText()).toBe('My checklist');
  ...
  it('updates the checklist title and emits CheckListTitleUpdated', () => {
    const checklist = CheckList.create(new IdCheckList(ID.generateId().getId()), owner, new Text('Initial'), actor, key);
    checklist.pullEvents();

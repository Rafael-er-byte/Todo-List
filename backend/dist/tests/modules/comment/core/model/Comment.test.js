...
const createCommentParams = (overrides) => ({
    id: DEFAULT_ID,
    creator: CREATOR_ID,
    idTask: TASK_ID,
...
const buildComment = (overrides) => {
    ...
    return Comment.create(new IdComment(params.id), new IdEntity(params.creator), new IdEntity(params.idTask), new Text(params.content), params.key);
};
const IDMock = {
...
describe("Comment Entity", () => {
    ...
    describe("Existence", () => {
        ...
        it("should return false when comment is deleted", () => {
            const comment = Comment.fromPrimitives(createCommentParams({ deletedAt: new Date() }));

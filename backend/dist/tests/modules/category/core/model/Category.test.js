...
const createCategoryParams = (overrides) => ({
    id: DEFAULT_ID,
...
const buildCategory = (overrides) => {
    ...
    return Category.create(params.key, new IdCategory(params.id), new CategoryName(params.name), new CategoryColor(params.color), new IdEntity(DEFAULT_ID), new IdEntity(params.idProject));
};
const IDMock = {
...
describe("Category Entity", () => {
    ...
    describe("Existence", () => {
        ...
        it("should delete a category", () => {
            const category = Category.fromPrimitives(createCategoryParams({ deletedAt: null }));
            category.pullEvents();
            expect(category.exists()).toBe(true);
            ...
            expect(() => category.updateName("test-key-5", new CategoryName("New Name"), IDMock)).toThrow(ResourceNotFound);
        });
    });

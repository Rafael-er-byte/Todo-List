...
describe("Project tests", () => {
    ...
    const DEFAULT_IMAGE = new Attachment(new Url("http://urlexample.com"), AllowedAttachments.jpeg, new Text("My image"), new IntNumber(200));
    it("Should create a project valid instance", () => {
        project = Project.create(new ProjectId(DEFAULT_ID), new ProjectName('Project 1'), new ProjectDescription("A project example"), new ProjectBackGroundColor(AllowedColors.BLUE), [], new ProjectSetting(AllowedProjectSetting.admins), false, new ProjectSetting(AllowedProjectSetting.admins), new ProjectSetting(AllowedProjectSetting.admins), true, new IdEntity(DEFAULT_ID), 'key-example');
        expect(project).toBeInstanceOf(Project);
        project = null;
        project = Project.create(new ProjectId(DEFAULT_ID), new ProjectName('Project 1'), new None(), new ProjectBackGroundImage(DEFAULT_IMAGE), [], new ProjectSetting(AllowedProjectSetting.member), false, new ProjectSetting(AllowedProjectSetting.member), new ProjectSetting(AllowedProjectSetting.member), true, new IdEntity(DEFAULT_ID), 'key-example');
        expect(project).toBeInstanceOf(Project);
    ...
    it("Should modify a closed project after available checks are removed", () => {
        function validateStatus() {
            expect(() => project.changeAddMemberSettings(new ProjectSetting(AllowedProjectSetting.admins), DEFAULT_KEY, new IdEntity(DEFAULT_ID))).not.toThrow();
    ...
    it("Should delete the project just if is closed before", () => {
        ...
        const events = project.pullEvents();
        expect((events[0]?.getActor()).getID()).toBe(DEFAULT_ID);

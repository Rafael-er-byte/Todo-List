...
describe('Attachment object tests', () => {
    ...
    it('Should throw if the attachment is not valid', () => {
        expect(() => new Attachment(validUrl, 'img', new Text('MyDoc'), new IntNumber(44444))).toThrow(LimitExceeded);
        expect(() => new Attachment(validUrl, 'another', new Text('MyDoc'), new IntNumber(2222))).toThrow(LimitExceeded);
    });
});

...
const buildTask = (id: string, position: number): Task => {
  const task = Task.create(
    new TaskTitle(`Task ${position}`),
    new IdEntity('0143c815-7220-7d64-8c42-6f2af4f9fd37'),
    new IntNumber(position),
    TaskState.pending(),
    false,
    new TaskId(id),
    projectId,
    ...
    new Collection([], [], []),
    actor,
    `task-created-${position}`,
  );
  task.pullEvents();
  return task;
};

const buildList = (tasks: Task[] = []): List => List.create(
